<script lang="ts" context="module">
	...
	export const load: Load = async ({ params, fetch, stuff }) => {
		...
		let url = `https://api.stockanalysis.com/wp-json/sa/financials?type=ratios&symbol=${symbol}&range=${
			range || 'annual'
		}`
...
<svelte:head>
	<title>{i.nameFull} ({i.ticker}) Financial Ratios and Metrics</title>
</svelte:head>

<FinancialsNavigation info={i} statement="ratios" period={range || 'annual'} />

<h1>Ratios and Metrics ({capitalize(range || 'annual')})</h1>

<script lang="ts" context="module">
	...
	export const load: Load = async ({ params, fetch, stuff }) => {
		...
		let url = `https://api.stockanalysis.com/wp-json/sa/financials?type=balance-sheet&symbol=${symbol}&range=${
			range || 'annual'
		}`
...
<svelte:head>
	<title>{i.nameFull} ({i.ticker}) Balance Sheet</title>
</svelte:head>

<FinancialsNavigation info={i} statement="balance-sheet" period={range || 'annual'} />

<h1>Balance Sheet ({capitalize(range || 'annual')})</h1>
